...
//     $stmt = $db['AS']->prepare($query_update);
//     $stmt->bind_param(
//         "ssssi",
//         $_POST["boardname"],
//         $_POST["boardsex"],
...
// $qstr = "SELECT boardid, boardname, boardsex, boardsubject,boardcontent FROM board WHERE boardid= ?";
// $result = $db['AS']->query($qstr);
//         //取出admin表中的username/password值
//         $$db['AS'] = $result->fetch_assoc();
//         $boardname = $row_result['boardname'];
//         $boardsex = $row_result['boardsex'];
//         $boardsubject = $row_result['boardsubject'];
//         $boardcontent = $row_result['boardcontent'];


// $db['AS']->close();
// $stmt = $db['AS']->prepare($query_RecBoard);
                ...
                <tr>
                    <td>
                        <div class="form-group form-inline">
                            ...
                            <input style="background-color:#CCDDFF" type="text" class="form-control col-8" name="boardname" id="boardname" value="<?php echo $row["boardname"]; ?>">
                        </div>
                        <div class="form-group form-inline">
                            <label for="boardsex">性別:</label>
                            <input style="background-color:#CCDDFF" name="boardsex" type="radio" id="radio" value="男" <?php if ($row["boardsex"] == "男") {
                                                                                                                            echo "checked";
                                                                                                                        } ?>>男
                            <input style="background-color:#CCDDFF" name="boardsex" type="radio" id="radio2" value="女" <?php if ($row["boardsex"] == "女") {
                                                                                                                            echo "checked";
                                                                                                                        } ?>>女
                        </div>
                        <div class="form-group form-inline">
                            <label for="boardsubject">標題:</label>
                            <input style="background-color:#CCDDFF" name="boardsubject" type="text" class="form-control col-8" id="boardsubject" value="<?php echo $row["boardsubject"]; ?>"></p>
                        </div>
                        <div class="form-group form-inline">
                            <label for="boardcontent">內容:</label>
                            <textarea style="background-color:#CCDDFF" name="boardcontent" id="boardcontent" class="form-control col-8" cols="30" rows="10"><?php echo $row["boardcontent"]; ?></textarea>
                        </div>
                    </td>

                    <td align="right">
                        <br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br>
                        <input name="boardid" type="hidden" id="boardid" value="<?php echo $row["boardid"]; ?>">
