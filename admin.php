...
<body>
    ...
    <!-- 頁中 -->
    <content>
        <?php while ($r = $db['AS']->next_record(1)) { ?>
            <table border="0" align="center" class="table table-success table-striped" style="max-width:1200px;" cellpadding="4" cellspacing="0">
                <tr valign="top">
                    <td width="60" align="center">
                        <?php if ($db['AS']->record['boardsex'] == "男") {; ?>
                            <img src="images/male.gif" alt="男" width="49" height="49">
                        <?php } else { ?>
                            <img src="images/female.gif" alt="女" width="49" height="49">
                        <?php } ?>
                        <br>
                        <span><?php echo $db['AS']->record['boardname']; ?></span>
                    </td>
                    <td>
                        <span style="font-size:22px;">
                            <font style="font-size:24px;color:blue;">主題: </font><?php echo $db['AS']->record['boardsubject']; ?>
                            <?php if ($db['AS']->record['checked'] == "1") {; ?>
                                <button align="right" class="btn btn-light checked" data-id="<?php echo $db['AS']->record['boardid'] ?>">
                                    <img src="images/love.gif" alt="已讀愛心" width="35" height="35">
                                </button>
                            <?php } else {; ?>
                                <button align="right" class="btn btn-light check" data-id="<?php echo $db['AS']->record['boardid'] ?>">
                                    <input name="boardid" type="hidden" id="boardid" value="<?php echo $db['AS']->record['boardid'] ?>">
                                    <input name="action" type="hidden" id="action" value="checked">
                                    <img src="images/unlove.gif" alt="未讀愛心" width="35" height="35">
                                </button>
                            <?php } ?>
                        </span>
                        <hr>
                        <p><?php echo nl2br($db['AS']->record['boardcontent']); ?></p>
                        <p align="right">
                            <?php echo $db['AS']->record['boardtime']; ?>
                        </p>
                        <a class="btn btn-dark" href="adminfix.php?id=<?php echo $db['AS']->record['boardid']; ?>">修改</a>
                        <a class="btn btn-danger" href="admindel.php?id=<?php echo $db['AS']->record['boardid']; ?>">刪除</a>
                    </td>
                </tr>
            </table>
        <?php } ?>
